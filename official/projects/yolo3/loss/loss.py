...
class YoloLoss(tf.keras.layers.Layer):
    ...
    def call(self, y_true, y_pred):
        ...
        pred_box, pred_obj, pred_class, pred_xywh = self.bbox(y_pred)
        ...
        def helperBroadcastIoU(box_1, box_2):
            # box_1: (..., (x1, y1, x2, y2))
            # box_2: (N, (x1, y1, x2, y2))
        ...
        best_iou = tf.map_fn(
            lambda x: tf.reduce_max(
                helperBroadcastIoU(x[0], tf.boolean_mask(x[1], tf.cast(x[2], tf.bool))), axis=-1),
